...
###############################################################
## v2.6.3 - Implemented multiplicative fitness               ##
##  - Implemented pre-selection epimutation as an option     ##
## rather than a separate script                             ##
## - Implemented LD calculation with lower/higher fitness    ##
## state with pop = 3 in record_LD                           ##
## (uses epigen_funcs_R1.4.1.R)                              ##
##  - Hacked in 50/50 starting freq when there               ##
## is no selection or epimutation                            ##
## - fixed a bug where record_gen was not being reset        ##
## for new workers on the same core                          ##
...
cat("sourcing functions from 'epigen_funcs_R1.4.1.R'\n")
source("epigen_funcs_R1.4.1.R")
...
spec = matrix(c(
    ...
    'mult_fitness',      'mf',   1, "logical",
    'pre_sel_epi',      'pe',    1, "logical",
...
### META-PARAMETERS ###
    ...
    --allo_gen | ag (Number of generations of allopatry between primary and secondary contact)
        When > 0 there is a period where migration between the two demes is set to zero
        after the burnin generations but before the contact of the neutral B allele.
        Default: 0
    ...
    --record_LD | rd (Types of linkage disequilibrium recorded each generation)
        Determine which allele pairs have their linkage disequilibrium (LD) calulated and recorded
        during simulations. A single instance of recording LD is specified as X_Y_Z. where X is the LD metric,
        Y is the pair of alleles and Z is the deme. Possible LD metrics are: 'D' (basic LD),
        'Dnorm' (normalized LD i.e. D'), and 'cov' (covariation). All allele pairs are possible and order
        does not matter e.g., AE is the same as EA. Possible demes are '1', '2', '0', and '3'. Deme zero averages
        over both demes. Deme 3 averages over both demes, swapping the alleles of the A and E loci for deme two.
        For example, D_EB_3 calculates the LD between E and B in deme one and e and B in deme two. This can be used
        to calculate LD between the neutral allele and alleles under selection. As a general example, calculating
        normalized LD between locus E and B in deme 1 is 'Dnorm_EB_1'. To calculate multiple types of LD,
        seperate multiple instances by commans e.g., 'Dnorm_EB_1,Dnorm_EB_2'.
        Default: ''
    ...
    --post_contact_mig | am (Enables migration after contact of the neutral B allele)
        ...
        Set to false and a small m to model analytical equations from Planidin et al. 2022.
        Default: T
    ...
    --inject_delay | id (Delay contact of the neutral allele X generations)
     ...
     --mult_fitness | mf (Use multiplicative fitness instead of additive fitness)
        If true, replaces additive fitness values with multiplicative fitness. With additive
        fitness, 1-s.e and 1-s.a are the fitnesses of a maladapted homozygote e.g., ee in deme 1.
        However, with multiplicative fitnesses, 1-s.e and 1-s.a are the fitnesses of the heterozygote
        e.g. eE in deme 1. The fitness of the maladapted homozygote is instead (1-s.e)^2. Note that
        there is no implementation of dominance or epistasis for multiplicative fitness at this time.
        Default: F
 
      --pre_sel_epi | pe (Have epimutation occur prior to selection during life history)
        If true, applies epimutation prior to selection during life history. Thus the life history
        is Migration > Epimutation > Selection > Mating / Recombination instead of 
        Migration > Selection > Mating / Recombination > Epimutation
        Default: F
...
### SIMULATION PARAMETERS ###
    ...
    --s.e | se (Selection at the epigenetic locus)
        ...
        These values assume no selection or epistasis from the genetic locus.
        Default: '0.5'
...
if(interactive()) {
    ...
    opt$equil_threshold   = 1e-12   
    opt$record_loci       = ""
    opt$record_LD         = "Dnorm_eB_3"
    opt$record_gen        = 1     
    ...
    # opt$function_string   = "param_mat[\"c_2\"]=param_mat[,\"i_1\"]"
    opt$function_string = ""
...
# filled in with the default values
    ...
    if(is.null(opt$allo_gen)         ) {opt$allo_gen          = 0      }
    ...
    if(is.null(opt$mult_fitness)     ) {opt$mult_fitness      = F      }
    if(is.null(opt$pre_sel_epi)      ) {opt$pre_sel_epi       = F      }
...
# during an interactive session
{
    ...
    allo_gen = as.numeric(opt$allo_gen)
    cat("Setting number of generations in allopatry to ",allo_gen,"\n")
    ...
    mult_fitness = as.logical(opt$mult_fitness)
    cat("Setting multiplicative fitness to ",mult_fitness,"\n")
    
    pre_sel_epi = as.logical(opt$pre_sel_epi)
    cat("Setting pre-selection epimutation to ",pre_sel_epi,"\n")
...
out_mat_list = foreach(i = 1:length(param_list_list),
    ...
    # Debug message
    # cat(paste0("Starting worker ", i, 
    #            ", cumulative_output_rows exists: ", 
    #            exists("cumulative_output_rows"), "\n"))
    # cat(paste0("Worker ", i, " starting with variables: ", 
    #            paste(ls(), collapse=", "), "\n"))        
    # prop_list = NULL
    ...
    if(mult_fitness) {
        param_mat = add_mult_fitness(param_mat,allele_combos)
    } else {
    ...
    # if there is only migration, sets the starting frequencies to
    # 50/50 in both demes
    # this is hacked in to speed up the otherwise slow approach
    # to equilibrium 
    only_mig <- apply(param_mat[,c("s.a","s.e","i_1","i_2","c_1","c_2"),drop = F],
                       1, function(row) all(row == 0))
    freq_mat[only_mig,c("freq_AA_EE_bb_1",
                        "freq_AA_EE_bb_2",
                        "freq_aa_ee_bb_1",
                        "freq_aa_ee_bb_2")] = 0.5
    
    # set up temporary variable to store record gen
    # which is updated in later generations to reduce
    # the recording frequency
    record_gen_tmp = record_gen
    ...
    while (cur_gen < max_gen) {
        ...
        if(cur_gen == (burnin + allo_gen + inject_delay)) {
            ...
            # debugging command to stop the while loop after
            # injection migration. post_injection_stop has to be set
            # manually in an interactive session
            # if(post_injection_stop) {stop()}
            # freq_mat_tmp1 = copy(freq_mat)
            
        # if not the contact generation, migration occurs as normal
        } else if (((cur_gen < burnin) & pre_contact_mig) |
                   ((cur_gen >= burnin + allo_gen) & post_contact_mig)) {
        ...
        if(equilibrium_threshold != 0) {
            ...
            } else if (cur_gen >= (burnin+allo_gen+inject_delay)){
        ...
        if(record_gen != 0 & cur_gen >= (burnin+allo_gen+inject_delay)) {
            
            # record less frequently in later generations to avoid making too large an output file
            if(cur_gen == (burnin + inject_delay + 100) & record_gen < 10) {record_gen_tmp = 10}
            if(cur_gen == (burnin + inject_delay + 1000) & record_gen < 100) {record_gen_tmp = 100}
            if(cur_gen == (burnin + inject_delay + 10000) & record_gen < 1000) {record_gen_tmp = 1000}
            
            if(cur_gen %% record_gen_tmp == 0) {
