...
##############################################
## v1.4.1 - Added pop == 3 in calc_LD       ##
## TO DO: implement dominance and epistasis ##
## for multiplicative fitness               ##
##############################################
...
# calculate the matrix of fitnesses based on selection parameters
# using multiplicative fitness
# note that there is no implementation of
# dominance or epistasis for multiplicative fitness yet
add_mult_fitness = function(param_mat,
                       allele_combos,
                       s = param_mat[,c("s.a","s.e")]) {
    ...
    # get the locus names from the fitness columns
    # e.g. s.e tells us its the fitness of e individuals in deme 1
    s_loci = sapply(1:length(strsplit(colnames(s),"\\.")), function(x) strsplit(colnames(s),"\\.")[[x]][2])
 
    # check that the names are valid
    # then create the alternate (epi)allele with toupper()
    if (sum(nchar(s_loci) - 1) != 0) {
        stop("Locus symbol is something other than a single letter")
    } else {
        allele1 = s_loci
        allele2 = toupper(s_loci)
    ...
    for(i in fitness_combos) {
        ...
        # calculate multiplicative fitness
        # note that there is no implementation of
        # dominance or epistasis for multiplicative fitness yet
        if(num_lc$a == 2 & num_lc$e == 2) {
            pop1_fit = (1 - s[,"s.a"])^2 * (1 - s[,"s.e"])^2
            pop2_fit = 1
        } else if (num_lc$a == 1 & num_lc$e == 2) {
            pop1_fit = (1 - s[,"s.a"]) * (1 - s[,"s.e"])^2
            pop2_fit = (1 - s[,"s.a"])
        } else if (num_lc$a == 0 & num_lc$e == 2) {
            pop1_fit = (1 - s[,"s.e"])^2
            pop2_fit = (1 - s[,"s.a"])^2
        } else if (num_lc$a == 2 & num_lc$e == 1) {
            pop1_fit = (1 - s[,"s.a"])^2 * (1 - s[,"s.e"])
            pop2_fit = (1 - s[,"s.e"])
        } else if (num_lc$a == 1 & num_lc$e == 1) {
            pop1_fit = (1 - s[,"s.a"]) * (1 - s[,"s.e"])
            pop2_fit = (1 - s[,"s.a"]) * (1 - s[,"s.e"])
        } else if (num_lc$a == 0 & num_lc$e == 1) {
            pop1_fit = (1 - s[,"s.e"])
            pop2_fit = (1 - s[,"s.a"])^2 * (1 - s[,"s.e"])
        } else if (num_lc$a == 2 & num_lc$e == 0) {
            pop1_fit = (1 - s[,"s.a"])^2
            pop2_fit = (1 - s[,"s.e"])^2
        } else if (num_lc$a == 1 & num_lc$e == 0) {
            pop1_fit = (1 - s[,"s.a"])
            pop2_fit = (1 - s[,"s.a"]) * (1 - s[,"s.e"])^2
        } else if (num_lc$a == 0 & num_lc$e == 0) {
            pop1_fit = 1
            pop2_fit = (1 - s[,"s.a"])^2 * (1 - s[,"s.e"])^2
        }
...
calc_LD = function(prop_list,
    ...
    for (i in record_LD) {
        ...
        # prop_list positions correspond to 
        # "aeb" "aeB" "aEb" "aEB" "Aeb" "AeB" "AEb" "AEB"
        
        # c("aeb","aeB","aEb","aEB","Aeb","AeB","AEb","AEB")[c(7,8,5,6,3,4,1,2)]
        
        # to get haplotype with A/a and E/e reversed
        # "AEb" "AEB" "Aeb" "AeB" "aEb" "aEB" "aeb" "aeB"
        # 7     8     5     6     3     4     1     2
        
        # to get haplotype with E/e reversed
        # "aEb" "aEB" "aeb" "aeB" "AEb" "AEB" "Aeb" "AeB"
        # 3     4     1     2     7     8     5     6
        # [c(3,4,1,2,7,8,5,6)]
        ...
        if (pop == 1) {
            ...
            haplo_freq_total = (prop_list$haplo_freq_pop1 + prop_list$haplo_freq_pop2[,c(7,8,5,6,3,4,1,2), drop = F]) / 2
        } else {
            stop("Invalid population for calculating LD, most be one of: 1 2 0 3")
        }
